...
echo <<<END
<div id="content">
<center><h3>Required Fields Missing</h3></center>
</div>
END;
else 
{
$query = "select * from customer_reg where cus_no like " . concat($_SESSION['cusno']) . " and password like " . concat($old_pass);
$res = mysql_query($query) or die('Query failed: ' . mysql_error());
$num = mysql_num_rows($res);
if($num>0)
{
$query = "update customer_reg set password = " . concat(md5($new_pass)) . " where cus_no like " . concat($_SESSION['cusno']) . " and password like " . concat($old_pass);
$res = mysql_query($query) or die('Query failed: ' . mysql_error());
if(!$res)
{
echo <<<END
<div id="content">
<center><h3>Error in Processing - Password Not Changed</h3></center>
</div>
END;
}
else
...
echo <<<END
<div id="content">
<center><h3>Invalid Old Password</h3></center>
</div>
END;
}
}
